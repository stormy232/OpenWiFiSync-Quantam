...
#define GPIO_OUT_W1TS_REG                                                      \
  0x3FF44008 // GPIO register for setting the output high
...
static bool IRAM_ATTR isr_oneshot_rising(gptimer_handle_t t,
                                         const gptimer_alarm_event_data_t *e,
                                         void *arg) {
  *gpio_out_w1ts_reg = (1U << SYNC_PIN);
...
static bool IRAM_ATTR isr_oneshot_falling(gptimer_handle_t t,
                                          const gptimer_alarm_event_data_t *e,
                                          void *arg) {
  *gpio_out_w1tc_reg = (1U << SYNC_PIN);
  gptimer_stop(t);
  return false;
}
...
esp_err_t timer_open(void) {
  if (!gptimerRisingEdge) {
    ...
    ESP_ERROR_CHECK(
        gptimer_register_event_callbacks(gptimerRisingEdge, &cb, NULL));
  ...
  if (!gptimerFallingEdge) {
    ...
    ESP_ERROR_CHECK(
        gptimer_register_event_callbacks(gptimerFallingEdge, &cb, NULL));

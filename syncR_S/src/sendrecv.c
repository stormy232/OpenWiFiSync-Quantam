...
void wifi_rx_cb(void *filterBuf, wifi_promiscuous_pkt_type_t type) {
  ...
  ESP_LOGD(TAG, "Received data from: " MACSTR " to: " MACSTR " len: %d",
           MAC2STR(src_addr), MAC2STR(dest_addr), combined_len);

  if ((memcmp(dest_addr, my_mac, MAC_LEN) != 0 &&
       memcmp(dest_addr, s_broadcast_mac, MAC_LEN) != 0)) {
  ...
  crc_cal =
      esp_crc16_le(UINT16_MAX, (uint8_t const *)data_in_recv_cb, combined_len);
  if (!(crc_cal == crc)) {
  ...
  if (msg_type == FOLLOW_UP) {
    ...
    if (setup_done) {
      if ((sPacket[countArray].count == mPacket.count) &&
          (sPacket[countArray].timeAP ==
           mPacket.timeAP)) { // timeAP check may be redundant in all honestly
                              // extra sec Ig
        sPacket[countArray].timeM = mPacket.timeM;
        sPacket[countArray].offset =
            sPacket[countArray].timeM - sPacket[countArray].timeS;

        if ((sPacket[countArray].timeS != 0) &&
            (sPacket[countArray].timeM != 0)) { // offset calc
...
void send_data(uint8_t *dest_mac, void *data, msg_type_t msg_type,
...
#if ENCRYPTION // * takes about 400 us when using maximum send length of 1426
               // bytes
  ...
  if (wifi_send_raw(dest_mac, send_filterBuffer, data_length + toBeAdded) !=
      ESP_OK) {
...
void calc_drift_task(void *pvParameter) {
  ...
  while (xQueueReceive(timeQueue, &count, portMAX_DELAY) == pdTRUE) {
    ...
    if (count > 0) {
      dividend =
          sPacket[count].offset - sPacket[count - 1].offset; //(s2-m2) - (s1-m1)
    ...
    sPacket[count].drift_filtered_ppm =
        ma_filter_add(sPacket[count].drift_unfiltered_ppm);
    timer_open();
    ...
    uint64_t ttt = toggle_time + sPacket[count].offset -
                   timeNow; // * time left until toggle
    double skew_abs =
        (double)ttt * 1.0e-6 *
        sPacket[count].drift_filtered_ppm; // * how much drift happens until
                                           // toggletimepoint

    ESP_LOGI("TAG", "GPTIMER_CONFIG");
    gptimer_alarm_config_t alarm_config = {
        .alarm_count =
            ttt + (int64_t)(skew_abs)-12, // * time estimate until toggle
    };
    ...
    gptimer_get_raw_count(gptimer, &timeNow);
    ESP_ERROR_CHECK(gptimer_set_raw_count(
        gptimer,
        timeNow + sPacket[count].offset)); // * estimate master time now

    ESP_LOGI(TAG, "ttt: %lld us, skew_abs: %.2f us", ttt, skew_abs);

    send_data(s_broadcast_mac, &sPacket[count], SYNC_VAL_OBS,
              sizeof(S_packet_t)); // * for logging on observer
  }
}
